defmodule Oban.Engines.QuackDBTest do
  ...
  test "executes jobs through an Oban supervision tree" do
    ...
    assert {:ok, job} = Oban.insert(name, Worker.new(%{ref: 1, action: "OK"}, queue: :default))
    assert_receive {:ok, 1}, 2_000

    assert eventually(fn ->
             match?(%Job{state: "completed"}, QuackRepo.get(Job, job.id))
           end)
  end
  ...
  test "inserts regular, bulk, and unique jobs", %{conf: conf} do
    ...
    assert length(Enum.uniq([first_id, second_id, third_id])) == 3
    ...
    assert %Job{args: %{"id" => 5, "version" => 2}} = QuackRepo.get!(Job, keyed_id)
    assert QuackRepo.aggregate(Job, :count) == 5
  end
  ...
  test "stages, fetches, completes, and records errors", %{conf: conf} do
    ...
    assert %Job{state: "completed", completed_at: %DateTime{}} = QuackRepo.get!(Job, executing.id)
  ...
  test "claims jobs once across concurrent producers", %{conf: conf} do
    ...
    tasks =
      for meta <- [first_meta, second_meta] do
        Task.async(fn -> QuackEngine.fetch_jobs(conf, meta, %{}) end)
      end
  ...
  test "cancels, retries, deletes, prunes, and rescues jobs", %{conf: conf} do
    ...
    assert :ok = QuackEngine.retry_job(conf, QuackRepo.get!(Job, available.id))
    assert %Job{state: "available"} = QuackRepo.get!(Job, available.id)

    assert {:ok, [%{id: completed_id}]} =
             QuackEngine.delete_all_jobs(conf, where(Job, [j], j.id == ^completed.id))

    assert completed_id == completed.id
    refute QuackRepo.get(Job, completed.id)

    old = DateTime.add(DateTime.utc_now(), -120)

    QuackRepo.update_all(
    ...
    refute QuackRepo.get(Job, available.id)
  end
