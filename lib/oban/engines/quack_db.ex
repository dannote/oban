defmodule Oban.Engines.QuackDB do
    ...
    case unique_query(changeset) do
      ...
      query ->
        ...
        with {:ok, result} <-
               Repo.transaction(
                 conf,
                 fn -> insert_unique(conf, changeset, query, inst_opts) end,
                 repo_opts
               ) do
          result
        end
  ...
  @impl Engine
  def fetch_jobs(_conf, %{paused: true} = meta, _running), do: {:ok, {meta, []}}

  def fetch_jobs(_conf, %{limit: limit} = meta, running) when map_size(running) >= limit,
    do: {:ok, {meta, []}}
  ...
  def fetch_jobs(%Config{} = conf, meta, running) do
    ...
    Repo.transaction(conf, fn ->
      ...
      query = where(Job, [j], j.id in subquery(subset) and j.state == "available")
  ...
  def discard_job(%Config{} = conf, %Job{} = job) do
    error = job |> Job.format_attempt() |> JSON.encode!()
    ...
    query =
      ...
      |> update([j],
        set: [
          ...
          errors: fragment("list_append(?, CAST(? AS JSON))", j.errors, ^error)
        ]
      )
  ...
  defp acquire_unique_lock!(conf) do
    case Repo.query!(conf, "UPDATE oban_locks SET revision = revision + 1 WHERE name = 'unique'") do
      %{num_rows: 1} -> :ok
  ...
  defp unique_query(%{changes: %{unique: %{} = unique}} = changeset) do
    ...
    states = states |> Job.cast_unique_group() |> Enum.map(&to_string/1)
